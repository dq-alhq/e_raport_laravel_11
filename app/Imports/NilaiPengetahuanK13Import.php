...
class NilaiPengetahuanK13Import implements ToCollection
{
    /**
     * @param array $row
     *
        ...
        foreach ($collection as $key => $row) {
            if ($key >= 10) {
                if ($row[6] > 0 && $row[6] <= 100) {
                    $cek_nilai = K13NilaiPengetahuan::where('k13_rencana_nilai_pengetahuan_id', $row[1])->where('anggota_kelas_id', $row[2])->first();
                    if (is_null($cek_nilai)) {
                        K13NilaiPengetahuan::create([
                            'k13_rencana_nilai_pengetahuan_id' => $row[1],
